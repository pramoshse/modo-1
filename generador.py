...
st.set_page_config(
    page_title="Generador de Procedimientos Modo 1",
    layout="wide",
)
...
def _mode_is_modo_1(mode: Any) -> bool:
    clean = _normalize(mode).lower().replace("°", "")
    return clean in {"modo 1", "modo1", "1", "m1"}
...
<head>
<meta charset="utf-8">
<title>Procedimiento Modo 1 - Control de Energías Peligrosas</title>
<style>
    @page {{
        size: A4 portrait;
        margin: 5mm;
...
st.title("GENERADOR DE PROCEDIMIENTOS MODO 1")
st.markdown(
    """
    <div class="soft-note">
...
with st.sidebar:
    ...
    codigo = st.text_input("Código", value="LOTO-M1-001")
...
if not _mode_is_modo_1(modo_final):
    st.markdown(
        f"""
        <div class="status-warn">
            El borrador importado no corresponde a <strong>Modo 1</strong>. Modo final detectado: <strong>{_html(modo_final or 'sin dato')}</strong>.
            Esta versión del generador solo habilita la plantilla Modo 1.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
procedure_html = build_modo_1_html(
    ctx,
    codigo=codigo,
    revision=revision,
    fecha=fecha,
...
st.markdown(
    """
    <div class="status-ok">
        Borrador compatible con <strong>Modo 1</strong>. Se generó la vista previa del procedimiento y quedaron habilitadas las descargas.
    </div>
    """,
    unsafe_allow_html=True,
)
...
with col_excel:
    try:
        excel_bytes = build_modo_1_excel_bytes(
            ctx,
            codigo=codigo,
            revision=revision,
            fecha=fecha,
